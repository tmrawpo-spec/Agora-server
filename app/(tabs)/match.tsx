...
export default function MatchScreen() {
  ...
  const { matchHistories, addMatchHistory, unlockMatchHistory } = useData();
  const {
    startMatching,
    stopMatching,
    setFoundUser,
  ...
  useEffect(() => {
    registerCancelHandler(cancelMatching);

    return () => {
      registerCancelHandler(null);
    };
  }, [registerCancelHandler, user?.id]);
  ...
  async function startFirestoreMatching() {
    ...
    try {
      ...
      for (const d of queueSnap.docs) {
        ...
        const data = d.data();
        if (matchedIds.includes(data.userId)) continue;
        if (data.matchedWith !== null) continue;
      ...
      if (foundMatch) {
        await setDoc(
          doc(db, "matchQueue", user.id),
          { matchedWith: foundMatch.userId },
          { merge: true }
        );
        await setDoc(
          doc(db, "matchQueue", foundMatch.userId),
          { matchedWith: user.id },
          { merge: true }
        );
        ...
        const profile = {
          ...
          location: "",
          distanceKm:
            myCoords && foundMatch.lat
              ? Math.round(
                  calcDistance(myCoords.lat, myCoords.lon, foundMatch.lat, foundMatch.lon)
                )
              : 0,
        ...
        matchListenerRef.current = onSnapshot(queueRef, async (snap) => {
          ...
          const profile = {
            ...
            location: "",
            distanceKm:
              myCoords && partnerData.lat
                ? Math.round(
                    calcDistance(myCoords.lat, myCoords.lon, partnerData.lat, partnerData.lon)
                  )
                : 0,
  ...
  async function handleUnlock(item: MatchHistory) {
    ...
    Alert.alert(
      ...
      lang === "ko"
        ? `${UNLOCK_COST} 코인으로 이 프로필을 공개할까요?`
        : `Use ${UNLOCK_COST} coins to unlock this profile?`,
      [
        { text: lang === "ko" ? "취소" : "Cancel", style: "cancel" },
        {
          text: lang === "ko" ? "공개하기" : "Unlock",
          onPress: async () => {
            const ok = await spendCoins(UNLOCK_COST);

            if (!ok) {
              Alert.alert(
                lang === "ko" ? "코인 부족" : "Not enough coins",
                lang === "ko"
                  ? `${UNLOCK_COST} 코인이 필요합니다.`
                  : `You need ${UNLOCK_COST} coins.`
              );
              return;
            }
    ...
    return (
      <View style={styles.historyItem}>
        <View style={styles.historyAvatar}>
          ...
          >
            <Ionicons
              name={item.profile.gender === "female" ? "female" : "male"}
              size={24}
              color="#fff"
            />
          </LinearGradient>
        ...
        <View style={styles.historyInfo}>
          <Text style={styles.historyName}>
            {isUnlocked ? item.profile.nickname : "???"}
          </Text>
        ...
        {matchState === "matched" && matchedProfile && (
          <View style={styles.matchedContainer}>
            ...
            <Text style={styles.matchedSub}>
              {matchedProfile.age}
              {t(lang, "years_old")} · {matchedProfile.distanceKm} km{" "}
              {lang === "ko" ? "거리에 있음" : "away"}
            </Text>
            ...
            <>
              <Text style={styles.historyHint}>
                {lang === "ko"
                  ...
                  : `First profile is free. Unlock others with ${UNLOCK_COST} coins.`}
              </Text>
              <FlatList
                data={matchHistories}
