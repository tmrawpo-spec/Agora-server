...
function PostCard({
  post, onPress, onLike, onImagePress, onBlock, lang
}: {
  post: Post;
  onPress: () => void;
  onLike: () => void;
  ...
  return (
    <Pressable style={styles.postCard} onPress={onPress}>
      ...
      <View style={styles.postHeader}>
        <Pressable onPress={handleProfilePress} style={styles.postAvatar}>
          {post.profilePhoto ? (
            ...
            <LinearGradient
              colors={post.gender === "female" ? [Colors.accent, "#c01f5d"] : [Colors.blue, "#2255aa"]}
      ...
      <View style={styles.postFooter}>
        ...
        <Pressable style={styles.blockPostBtn} onPress={onBlock}>
          <Ionicons name="ban-outline" size={14} color={Colors.danger} />
          <Text style={styles.blockPostBtnText}>차단/신고</Text>
        </Pressable>
      </View>
    </Pressable>
  );
...
export default function BoardScreen() {
  ...
  async function togglePreview() {
    ...
    try {
      ...
      Alert.alert("Error", lang === "ko" ? "재생 실패" : "Playback failed");
    }
  }
  ...
  async function handleBlock(withReport: boolean) {
    ...
    if (user?.id && blockTargetUserId) {
      try {
        await updateDoc(doc(db, "users", user.id), {
          blockedUsers: arrayUnion(blockTargetUserId),
        });
        if (withReport && blockReason) {
          await addDoc(collection(db, "reports"), {
            reporterId: user.id,
            reportedId: blockTargetUserId,
            reason: blockReason,
            createdAt: Date.now(),
          });
        }
  ...
  const sorted =
    boardTab === "popular"
      ? [...posts].filter(p => p.likes >= 5 || p.isPopular).sort((a, b) => b.likes - a.likes)
  ...
  return (
    <View style={[styles.container, { paddingTop: Platform.OS === "web" ? 67 : insets.top }]}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <PostCard
            ...
            onBlock={() => {
              setBlockTargetUserId(item.userId);
        ...
        <View style={[styles.compose, { paddingTop: insets.top }]}>
          <View style={styles.composeHeader}>
            ...
            <Text style={styles.composeTitle}>
              {lang === "ko" ? "새 게시글" : lang === "zh" ? "新帖子" : "New Post"}
            </Text>
          ...
          <ScrollView style={styles.composeBody} keyboardShouldPersistTaps="handled">
            {photo && (
              <View style={styles.photoPreviewWrap}>
                ...
                <Pressable style={styles.removePhoto} onPress={() => setPhoto(undefined)}>
            ...
            {voiceUri && (
              <View style={styles.voicePreview}>
                ...
                <Pressable style={{ marginLeft: "auto" }} onPress={() => setVoiceUri(undefined)}>
            ...
            <View style={styles.recordingOverlay}>
              ...
              <View style={styles.recordingContent}>
                ...
                <Text style={styles.recordingText}>
                  {lang === "ko" ? "녹음 중..." : lang === "zh" ? "录音中..." : "Recording..."}
                </Text>
                <Text style={styles.recordingSubText}>
                  {lang === "ko" ? "손을 떼면 완료됩니다" : lang === "zh" ? "松手结束" : "Release to finish"}
                </Text>
              </View>
            </View>
          )}
      ...
      <BlockReportModal
        ...
        onClose={() => { setShowBlockModal(false); setBlockReason(null); setBlockTargetUserId(null); }}
...
const styles = StyleSheet.create({
  ...
  postFooter: { flexDirection: "row", gap: 16, paddingTop: 4, alignItems: "center" },
  ...
  blockPostBtn: { flexDirection: "row", alignItems: "center", gap: 4, marginLeft: "auto" },
  blockPostBtnText: { color: Colors.danger, fontSize: 12, fontWeight: "600" },
